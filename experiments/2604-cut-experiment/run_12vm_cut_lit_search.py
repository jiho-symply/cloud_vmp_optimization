...
ANALYSIS_DIR = EXPERIMENT_DIR / "results" / "analysis" / "cut_12vm_od_sp_bj_cap8_avg20_lit_search"
...
def parse_args():
    parser = argparse.ArgumentParser(description="12VM cut screening 후 상위 후보를 장시간 재평가합니다.")
    ...
    parser.add_argument("--screen-time-limit", type=int, default=1200)
    parser.add_argument("--final-time-limit", type=int, default=3600)
...
def run_stage(stage_name, case_name, instance_dir, profiles, time_limit, mip_gap, max_workers, threads):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(run_single_job, task): task[3]
            for task in tasks
        }
...
def create_metric_bar(summary_df, output_path, metric, ylabel, color):
    ...
    ax.bar(plot_df["profile"], plot_df[metric], color=color, edgecolor="#1F1F1F", linewidth=0.9)
...
def write_markdown_report(screen_df, final_df, finalists, output_path, screen_time_limit, final_time_limit):
    lines = [
        ...
        "- 목적함수: 에너지 최소화",
        f"- 1차 screening: 각 단일 컷 또는 단일 solver 전략을 `{screen_time_limit}초`로 비교",
        f"- 2차 재평가: screening 상위 후보와 `baseline`을 `{final_time_limit}초`로 재평가",
        "- 조합 프로파일은 제외하고 단일 프로파일만 비교",
        "",
        "## 최종 재평가 대상",
        "",
    ]
...
def main():
    ...
    ranked_screen_csv = ANALYSIS_DIR / "screen_ranking.csv"
    ranked_screen.to_csv(ranked_screen_csv, index=False)
    ...
    print(f"saved: {ranked_screen_csv}")
